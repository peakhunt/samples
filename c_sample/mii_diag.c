...
int
mdio_read(int skfd, int phy_id, int location)
{
   u16 *data = (u16 *)(&ifr.ifr_data);
...
void
mdio_write(int skfd, int phy_id, int location, int value)
{
   u16 *data = (u16 *)(&ifr.ifr_data);
...
static int
parse_advertise(const char *capabilities)
{
   const char *mtypes[] =
   {
...
int
show_basic_mii(long ioaddr, int phy_id)
{
   int mii_reg, i;
   u16 mii_val[32];
   ...
   if (lkpar & 0x4000)
   {
      printf(" Your link partner advertised %4.4x:", lkpar);
      for (i = 5; i >= 0; i--)
         if (lkpar & (0x20<<i))
...
#ifndef LIBMII
extern void
monitor_mii(long ioaddr, int phy_id)
{
   fprintf(stderr,
